...
# Streamlit components for file upload and text area input
st.header("Upload Resume as a PDF file")
uploaded_file = st.file_uploader("Choose a PDF file", type="pdf")
...
if st.button("Check Match Score"):
    ...
    if job_description:
        try:
            ...
            st.error(f"An error occurred while processing the job description: {e}")
    else:
    ...
    if resume and jd:
        try:
            ...
            if resume_skills and jd_skills:
                ...
                st.warning("Could not compare skills due to missing data.")
        except Exception as e:
            st.error(f"An error occurred during skill extraction or comparison: {e}")
    else:
